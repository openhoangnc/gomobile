...
GOOS=darwin GOARCH=arm64 GOFLAGS=-tags=ios,macos,catalyst CC=macosx-clang CXX=macosx-clang++ CGO_CFLAGS=-isysroot=macosx -target arm64-apple-ios13.0-macabi {{if .BitcodeEnabled}}-fembed-bitcode {{end}}-arch arm64 CGO_CXXFLAGS=-isysroot=macosx -target arm64-apple-ios13.0-macabi {{if .BitcodeEnabled}}-fembed-bitcode {{end}}-arch arm64 CGO_LDFLAGS=-isysroot=macosx -target arm64-apple-ios13.0-macabi {{if .BitcodeEnabled}}-fembed-bitcode {{end}}-arch arm64 CGO_ENABLED=1 go build -tags tag1 -x {{if .Main}}-ldflags=-w -o=$WORK/catalyst/arm64 {{end}}{{.Pkg}}
GOOS=darwin GOARCH=amd64 GOFLAGS=-tags=ios,macos,catalyst CC=macosx-clang CXX=macosx-clang++ CGO_CFLAGS=-isysroot=macosx -target x86_64-apple-ios13.0-macabi {{if .BitcodeEnabled}}-fembed-bitcode {{end}}-arch x86_64 CGO_CXXFLAGS=-isysroot=macosx -target x86_64-apple-ios13.0-macabi {{if .BitcodeEnabled}}-fembed-bitcode {{end}}-arch x86_64 CGO_LDFLAGS=-isysroot=macosx -target x86_64-apple-ios13.0-macabi {{if .BitcodeEnabled}}-fembed-bitcode {{end}}-arch x86_64 CGO_ENABLED=1 go build -tags tag1 -x {{if .Main}}-ldflags=-w -o=$WORK/catalyst/amd64 {{end}}{{.Pkg}}
...
GOOS=darwin GOARCH=amd64 GOFLAGS=-tags=macos CC=macosx-clang CXX=macosx-clang++ CGO_CFLAGS=-isysroot=macosx {{if .BitcodeEnabled}}-fembed-bitcode {{end}}-arch x86_64 CGO_CXXFLAGS=-isysroot=macosx {{if .BitcodeEnabled}}-fembed-bitcode {{end}}-arch x86_64 CGO_LDFLAGS=-isysroot=macosx {{if .BitcodeEnabled}}-fembed-bitcode {{end}}-arch x86_64 CGO_ENABLED=1 go build -tags tag1 -x {{if .Main}}-ldflags=-w -o=$WORK/macos/amd64 {{end}}{{.Pkg}}
xcrun lipo -o $WORK/main/main -create $WORK/simulator/arm64 $WORK/simulator/amd64 $WORK/ios/arm64 $WORK/catalyst/arm64 $WORK/catalyst/amd64 $WORK/macos/arm64 $WORK/macos/amd64
